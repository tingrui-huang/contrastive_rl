...
def native(params, actor, reset_seeds):
    ...
    for _ in range(50):
        observation = np.concatenate([states[-1], np.tile(GOAL, (len(sims), 1))], -1)
        # Keep GEMM shape identical to the main evaluation during small replays.
        # Padding is for a feed-forward batch only; no fictitious trajectory is stored.
        assert 0 < len(sims) <= 128
        padded = np.pad(observation, ((0, 128-len(sims)), (0, 0)))
        action = np.asarray(actor(params, padded))[:len(sims)]
...
def evaluate(root):
    ...
    native_steps = amendment.get('failed_attempt_native_steps', 0)
    ...
    assert native_steps <= amendment.get('revised_native_step_cap', CONFIG['native_step_cap'])
    ...
    write(root/'evaluation.json', dict(native=summaries, native_paired=comparisons,
        ...
        engineering_amendment=amendment,
