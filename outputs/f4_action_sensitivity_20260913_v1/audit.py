...
def ett_phase(contexts):
    ...
    indexes=np.repeat(np.arange(len(panel)),20);es=np.tile(np.repeat(EPS,4),len(panel));dirs=np.tile(np.arange(4),len(panel)*len(EPS))
...
def main():
    if '--resume-models' in sys.argv:
        previous=json.loads((OUT/'costs.json').read_text())
        COUNTS.update(previous['counts'])
        assert COUNTS['model']==752 and COUNTS['jacobian']==240
        provenance=json.loads((OUT/'provenance.json').read_text())
        assert all(sha(ROOT/p)==h for p,h in provenance['source_sha256'].items())
        write('index_fix.json',dict(reason='Direction index repeated across contexts but not epsilons; failed before first ETT successor call',
              old_source_sha256=sha(OUT/'audit_before_index_fix.py'),new_source_sha256=sha(__file__),
              original_failure_preserved=True,repeat_native_or_deterministic_calls=0,counts_at_resume=COUNTS.copy()))
        with (OUT/'contexts.pkl').open('rb') as stream:contexts=pickle.load(stream)
        ett_phase(contexts)
        assert all(sha(ROOT/p)==h for p,h in provenance['source_sha256'].items())
        progress('complete')
