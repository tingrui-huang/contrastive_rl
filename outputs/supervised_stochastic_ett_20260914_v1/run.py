...
def main():
    if '--correct-legality' in sys.argv:
        import shutil
        from crl.envs import TwoRouteSwampWindyF4Env
        prior=OUT/'before_legality_correction';assert not prior.exists();prior.mkdir()
        for name in ['metrics.json','gate.json','completion.json','rollout_status.json','costs.json']:
            shutil.copyfile(OUT/name,prior/name)
        COUNTS.update(json.loads((OUT/'costs.json').read_text())['counts'])
        provenance=json.loads((OUT/'provenance.json').read_text())
        assert all(sha(p)==h for p,h in provenance['sources'].items())
        d=dict(np.load(DATA));eq=np.all(d['xb']==d['xq'],axis=1)
        finals={'initial':np.zeros(48,np.float32)}
        for seed in [0,1]:
            for arm in ['A','B']:finals[f'{arm}_s{seed}']=np.load(OUT/f'{arm}_s{seed}.npz')['theta']
        env=TwoRouteSwampWindyF4Env();counts={}
        for name in finals:
            path=OUT/(name+'_evaluation.npz');shutil.copyfile(path,prior/path.name)
            saved=dict(np.load(path));xy=saved['samples_xy'];old=saved['legal'].copy()
            saved['legal']=legal(xy);saved['mean_legal']=legal(saved['mean'])
            changed=np.unique(xy[old!=saved['legal']],axis=0)
            assert all(not env._is_blocked(point) for point in changed)
            controls=xy.reshape(-1,2)[::601]
            assert np.array_equal(legal(controls),[not env._is_blocked(p) for p in controls])
            counts[name]=dict(changed_sample_labels=int(np.sum(old!=saved['legal'])),
                             native_checked_unique_changed_positions=len(changed),native_checked_controls=len(controls))
            np.savez_compressed(path,**saved)
        write('legality_correction.json',dict(reason='Native boundary test is > high, not >= high; outer X=9 is valid in the upper corridor',
              production_changed=False,thresholds_changed=False,training_repeated=False,samples_regenerated=False,
              old_driver_sha256=sha(OUT/'run_before_legality_fix.py'),new_driver_sha256=sha(__file__),checks=counts))
        passed=summarize(d,eq,finals)
        if passed:rollout(Kernel(),finals)
        else:write('rollout_status.json',dict(ran=False,reason='Corrected one-step gate failed'))
        assert all(sha(p)==h for p,h in provenance['sources'].items())
        progress('complete');write('completion.json',dict(status='complete',costs=COUNTS,source_hashes_unchanged=True,
              ...
              legality_corrected_to_native_semantics=True))
        return
