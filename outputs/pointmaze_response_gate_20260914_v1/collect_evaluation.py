"""Collect the one new paired PointMaze evaluation set with frozen seeds."""
import argparse
import copy
import hashlib
import json
...
def main():
    ...
    for episode in range(args.episodes):
        env = TwoRouteSwampWindyF4Env(
            seed=args.seed + episode, active_prob=0.3
        )
        ...
        for time in range(50):
            xb = np.asarray(
                teacher(env.state.copy(), env.goal.copy(), teacher_memo),
                np.float32,
            )
            if np.any(xb != 0):
                xb = np.clip(
                    xb + teacher_rng.normal(0, 0.15, 2), -1, 1
                ).astype(np.float32)
            forward = np.asarray(
            ...
            for query, action in enumerate(actions):
                ...
                values = dict(
                    s=observation[:8].copy(), xb=xb.copy(), xq=action.copy(),
                    y=next_observation[:8].copy(), episode=episode, time=time,
                    query=query, prefix_mode=prefix_mode, dead_before=env.dead,
                    dead_after=branch.dead, reward=reward,
                )
