...
# Env-overridable so the same converter serves the local workstation and Colab
# (which sets D4RL_HDF5=/content/...hdf5 and OFFLINE_NPZ=/content/...npz).
DATA = os.environ.get(
    'D4RL_HDF5', r'D:\Users\trhua\Research\datasets\d4rl\antmaze-umaze-v2.hdf5')
OUT = os.environ.get(
    'OFFLINE_NPZ',
...
def convert(hdf5_path=None, out_path=None, report_dir='artifacts/offline_d4rl'):
  """Convert an antmaze-umaze-v2 hdf5 to the crl offline .npz episode format.

  Same logic as before; parameterized so Colab can call it with /content paths.
  Returns the conversion meta dict."""
  import h5py
  hdf5_path = hdf5_path or DATA
  ...
  meta = {
      'source_hdf5': hdf5_path, 'out': out_path, 'episodes_kept': E,
      ...
      'out_size_mb': round(os.path.getsize(out_path) / 1e6, 1),
  }
  if report_dir:
    os.makedirs(report_dir, exist_ok=True)
    json.dump(meta, open(os.path.join(report_dir, 'npz_conversion.json'), 'w'),
              indent=2)
