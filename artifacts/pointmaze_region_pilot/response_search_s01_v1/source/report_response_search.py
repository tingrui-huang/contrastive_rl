...
def run(out):
    # Reporting-only correction after sampling; preserve the sealed source and
    # verify all other experiment inputs/code without relaxing their hashes.
    declaration=p.read(out/'preregistration.json')
    for f,h in declaration['sources'].items():
        if f=='ett/report_response_search.py' and sha(f)!=h:
            fix=p.read(out/'reporting_fix.json')
            assert sha(out/'sealed_report_source.py')==h==fix['original_sha256']
            assert sha(f)==fix['corrected_sha256']
        else:assert sha(f)==h,f
    for f,h in declaration['sealed'].items():assert sha(out/f)==h,f
    done=p.read(out/'completion.json');search=p.read(out/'search.json');selection=p.read(out/'selection.json')
