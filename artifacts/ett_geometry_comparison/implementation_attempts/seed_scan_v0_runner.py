...
def prepare(root):
    ...
    hits=[line for line in search.stdout.splitlines() if not line.startswith(('ett\\run_geometry_comparison.py:', 'ett/run_geometry_comparison.py:'))]
    ...
    write(root/'seed_audit.json',dict(query='exact proposed namespace bases in existing JSON/Python/SPEC artifacts',
        prior_matches=hits,scope='repository artifacts, ett, scripts, notes; current new runner excluded',
