...
- `s0_L0p25_lambda1`: diagonal NLL -5.799028; energy 0.013710; MMD K8/K16 0.737583/0.724239; moving-history motion 0.37603; moving-history near-zero fraction 2.3537%; mean XY std [0.18997, 0.1885].
- `s0_L0p75_lambda1`: diagonal NLL -5.799089; energy 0.013709; MMD K8/K16 0.698687/0.682470; moving-history motion 0.40881; moving-history near-zero fraction 5.7141%; mean XY std [0.1969, 0.23178].
...
- `s1_L0p25_lambda1`: diagonal NLL -5.804538; energy 0.013716; MMD K8/K16 0.738637/0.725316; moving-history motion 0.37309; moving-history near-zero fraction 2.3899%; mean XY std [0.18987, 0.18761].
...
For the predeclared primary arm `s0_L0p25_lambda1`, fixed historical frames contribute 70.51% of aggregate cross-reference squared distance. Mean nearest-reference fixed-history squared distance is 5.31032. This contribution cannot be changed in one step. Moving histories cannot become fully frozen in a single valid F4 transition; lower full-F4 MMD does not imply death.

At K=16 the editable-XY MMD gradient norm averages 0.275126; 0.00% of contexts have norms below 1e-8. The smallest kernels can saturate for far-away histories; larger kernels retain gradients. The kernel is fixed, and full-F4 matching was never replaced by newest-XY matching.

Primary-arm candidate geometry corrections: 2.3804%; bound-violation fallback: 8.0139%; blocked straight-line segments (21-point probe): 0.0183%. Every evaluated arm and both K values had zero final endpoint violations, zero history-shift error and zero final bound violations. Corrective endpoint geometry is still an approximation of the environment axiswise collision substeps, and the segment probe is not a physics proof.
...
python -m ett.run_distribution_matching --out-dir artifacts/ett_distribution_matching/f4_p30_s01 --steps 500 --eval-every 100 --batch-size 64 --eval-contexts 1024 --diagonal-samples 64 --bounds 0.25,0.75 --seeds 0,1
python -m ett.report_distribution_matching --run-dir artifacts/ett_distribution_matching/f4_p30_s01
```

The earlier numerical checks are in sibling `sanity` and `sanity_v2` directories. The first used one shared Adam rate and exposed a sharp diagonal update; the second verified separate rates before the bounded comparison. Neither smoke result selected lambda/L by validation performance.
...
model = load_anchored('artifacts/ett_distribution_matching/f4_p30_s01/s0_L0p25_lambda1/final.pkl')
