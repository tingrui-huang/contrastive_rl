# Constrained fixed-actor ETT adversarial prototype
...
Lambda is fixed at 1/sum(.95^t) = 0.05416795, scaling J into [0,1]. With the diagonal law invariant, this controls update scale rather than a diagonal/return tradeoff. Each of optimizer seeds 0/1 compares lambda=0 to that single positive weight, from exactly zero coefficients, using matched query streams. Sixteen updates use eight directions, sigma=.10, 32 paths per signed query, learning rate 1 and update norm cap .20. The final iterate is always used. Monitors never select checkpoints; the two control parameter arrays and independent evaluation paths are identical.
...
- Seed 0: control return 10.51672, adversarial 10.14998; paired difference -0.36673, 95% interval [-0.60671, -0.13723].
- Seed 1: control return 10.51672, adversarial 10.06960; paired difference -0.44712, 95% interval [-0.65304, -0.23360].
...
Diagonal coupled output drift and maximum per-tuple energy-score increase are exactly 0 on all 2,048 training and 2,048 held-out tuples for every arm, within budgets 2e-6 and 1e-6. The held-out energy score stays 0.01361403. Held-out predictive-mean XY L2 errors before and after have median/p95/p99/max [0.012497222051024437, 0.0499434625729917, 0.29884670346975284, 0.9982609748840332]; the near-1 maximum shows existing tail error despite excellent mean fit. Training energy remains 0.01257015. The unprojected held-out diagnostic NLL remains -5.884551; it is not the emitted likelihood.

Initialization was independently checked against the original diagonal sampler; the maximum difference was 0.0. Global diagonal identity follows from the construction, not from these finite rows alone.

## Action response, boundary behavior and remaining failure modes

- Control: maximum audited action ratio 0.0000; positive bound excess 0; exact action-invariant grid fraction 100.00%; mean action spread 0.0000.
- Adversarial seed 0: maximum audited action ratio 0.2036; positive bound excess 0; exact action-invariant grid fraction 0.00%; mean action spread 0.1389.
- Adversarial seed 1: maximum audited action ratio 0.2853; positive bound excess 0; exact action-invariant grid fraction 0.00%; mean action spread 0.1796.
...
Completed 1,862,400 primary model transitions and 869,376 fit/audit/replay transitions, plus a 10,000-sample test reserve (including the independent 8,196-sample initialization/shape check), below the 2,862,400 cap. Search/monitoring took 12.34 seconds after initial compilation and fit checks on local CPU. Five focused tests pass; 512 saved full evaluation paths replay exactly. Saved rewards, F4, action bounds, paired intervals and frozen hashes independently verify. All checkpoints stay in ignored checkpoints/. Reports and evaluation arrays are English and publishable.

```bash
python -m unittest scripts.test_convex_action_transition
